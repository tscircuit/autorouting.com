...
export async function uploadDatasetOutputs({
 ...
 for (const outputFile of outputFiles) {
       ...
       const relativeFilePath = outputFile.substring(outputsDir.length + 1).replace(/\\/g, '/')
       ...
       try {
         ...
         debug(`File doesn't exist: outputs/${relativeFilePath} for sample ${sampleNumber}`)
       }

       // Get the sample and its available files
       const { sample } = await ky
         .get<{ sample: Sample & { available_file_paths: string[] } }>("samples/get", {
           searchParams: {
             dataset_id: dataset.dataset.dataset_id,
             sample_number: sampleNumber,
           },
       ...
       const endpoint = fileExists ? "samples/update_file" : "samples/create_file"
     ...
     const filesToDelete = sample.available_file_paths.filter(
       (remotePath) => 
         remotePath.includes("_routed") && 
         !localFiles.includes(remotePath)
     )

     // Delete each file that no longer exists locally
     for (const fileToDelete of filesToDelete) {
       debug(`Deleting remote file that no longer exists locally: ${fileToDelete}`)
